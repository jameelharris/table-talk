...
_MARK_MESSAGE = "mark-pending: rebuilding hand_starts"


def _batch_row(index, status="failed_transient", status_message=_MARK_MESSAGE):
    return HandSetupProcessingAttemptsRow(
        attempt_id=f"attempt_{index:03d}",
        hand_setup_id=f"dQw4w9WgXcQ_001_{index:03d}",
        status=status,
...
def test_batch_emits_one_insert_with_a_tuple_per_row():
    ...
    param_names = {p.name for p in kwargs["job_config"].query_parameters}
    assert "hand_setup_id_0" in param_names
    assert "hand_setup_id_1" in param_names
...
def test_batch_empty_list_is_a_noop():
    ...
    write_hand_setup_processing_attempt_rows([], project="proj", dataset="ds", client=mock_client)
...
def test_batch_rows_disagreeing_on_columns_raise():
    ...
    with pytest.raises(HandSetupProcessingAttemptsWriteError, match="disagree on columns"):
        write_hand_setup_processing_attempt_rows(
