...
_MARK_MESSAGE = "mark-pending: rebuilding tournament_results"
...
def test_batch_emits_one_insert_with_a_tuple_per_row():
    ...
    assert "proj.ds.tournament_results_processing_attempts" in query_str
...
def test_batch_empty_list_is_a_noop():
    ...
    write_tournament_results_processing_attempt_rows(
        [], project="proj", dataset="ds", client=mock_client
    )
...
def test_batch_rows_disagreeing_on_columns_raise():
    ...
    with pytest.raises(TournamentResultsProcessingAttemptsWriteError, match="disagree on columns"):
        write_tournament_results_processing_attempt_rows(
