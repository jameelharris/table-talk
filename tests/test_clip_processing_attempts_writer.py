...
_MARK_MESSAGE = "mark-pending: rebuilding hand_setups"


def _batch_row(index, status="failed_transient", status_message=_MARK_MESSAGE):
    return ClipProcessingAttemptsRow(
        clip_id=f"dQw4w9WgXcQ_{index:03d}",
        status=status,
...
def test_batch_emits_one_insert_with_a_tuple_per_row():
    ...
    param_names = {p.name for p in kwargs["job_config"].query_parameters}
    assert "clip_id_0" in param_names
    assert "clip_id_1" in param_names
...
def test_batch_empty_list_is_a_noop():
    ...
    write_clip_processing_attempt_rows([], project="proj", dataset="ds", client=mock_client)
...
def test_batch_rows_disagreeing_on_columns_raise():
    ...
    with pytest.raises(ClipProcessingAttemptsWriteError, match="disagree on columns"):
        write_clip_processing_attempt_rows(
