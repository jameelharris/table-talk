...
Established by exhaustive validation of 65 hands in video `MPBLfM4mwfE` (60 `complete` with rows, 1 uncontested, 2 `complete_skipped`, 2 `failed_transient`). Downstream phases must treat all of the following as **normal input**, not defects:
...
Two corpus hands motivated this. `MPBLfM4mwfE_008_004` failed three times with `no_first_voluntary_commitment_found`, including after the step-A prompt fix. `MPBLfM4mwfE_004_008` failed once the same way, then twice with `no second action observed within window` — the step-A fix moved it from "cannot find the FVA" to "found the FVA, cannot find the second action." **These are two different failure classes, not one pattern**, and should not be diagnosed as one when the parked sample grows.
