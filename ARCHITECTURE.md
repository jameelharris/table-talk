...
- **Clip-mode video scans — inconclusive.** Flash truncated a street on six hands that both Pro runs found — five rivers and one turn. Four were runouts where the cards are analytically inert but still needed for showdown evaluation and duplicate-card detection. **One, t=1320, had betting on the river that Flash therefore dropped entirely** — a lost decision, not a lost card. That is the entire case for Pro on clips, and it is weaker than it reads: only t=1320 was verified against the broadcast, and two Pro runs agreeing is thin corroboration from a pair that also agreed with each other while both were wrong on seat labels at t=984 and t=1184. The reproduction that would separate "Flash cannot find these windows" from "Flash was unlucky six times" was never run. On the same call mode the evidence points the other way too: Pro produced 9 FVA boundary violations to Flash's 0, fabricated a four-street runout at t=1779, and parked three hands on card reads against Flash's one. **This bullet read "Clip-mode video scans — Pro" until the clip default was reconsidered; that was a stronger claim than the evidence carries, and nothing here establishes the converse either.**

**The clip default is Flash, and it is a cost and throughput decision.** Pro's clip-mode calls measured **68,874 prompt tokens** each against roughly 2,300 for a frame read — about 30x, on the pipeline's highest-volume call type. Clip 009 hit a Vertex 429 twice at `--max-concurrent 2`, exhausted the internal backoff both times and parked, needing a manual un-park and a third run at `--max-concurrent 1`. That happened on a single video, and the quota pressure recurs on every one. Against it, the projected quality cost at a 20K-hand corpus is roughly 1.5% of hands with an incomplete board and a few hundred with a genuinely lost betting street: tolerable for population-tendency analysis, and bounded, where the quota pressure is not.

**The quality cost is detectable, which is most of why it is acceptable.** Step E records a D/E street disagreement in `status_message` as `D reported <street> but 2 scans found none — truncated there`, so truncated hands are a queryable population rather than silent loss. See "Scan retries fire only on disagreement with D."

**The split is retained so this is reversible.** `TT_CLIP_MODEL=gemini-2.5-pro` moves clip calls back without touching frame reads, and that reversibility is why two constants survive a change that makes their defaults identical — collapsing them would trade a one-variable rollback for nothing. Revisit after the first multi-video ingest, when there is a truncation rate measured across broadcasts rather than six cases on one video.
...
- **Revisit the clip-mode model default after the first multi-video ingest** — `TT_CLIP_MODEL` defaults to Flash on a cost and throughput argument whose quality side is six truncations on one video, only one of them adjudicated. A second broadcast gives a truncation rate rather than a case count, and `status_message` already records the hands to count. See "Model selection is per call mode"; the rollback is one environment variable.
