...
Picks up pending clips from `clip_manifest`, downloads the source video to a per-video local tempfile, makes a clip-mode Gemini call on each clip to identify hand setup moments, extracts a frame at each moment via ffmpeg, makes a frame-mode call on each frame to extract player info, enriches with deterministic seat metadata, and writes `hand_setups` rows + frame JPEGs to GCS. On progressive-knockout videos the frame prompt carries a bounty addendum and each player gains a `bounty`. All work per clip is atomic. Defines hand setup moments only; the first voluntary action is Phase 4 and per-hand action sequences are later.
...
Downstream must still *derive* rather than extract two things. Per-seat forced contributions: the ante coefficient is `(pot_size_bb − 1.5) / total_seat_count`, fitted per video by median so a misread pot does not drag it. `MPBLfM4mwfE` fits 0.125 exactly across seat counts 9 through 2; `YzKyFMQ1avU` clusters 0.120–0.130, so **0.125 is that video's value and not a constant** — see "The ante coefficient is per video." Note it is a **big blind ante** — the total collected is `ante × seats`, posted by one player, so `committed[seat]` is 0.5 for the SB, 1.0 plus the full table ante for the BB, and zero for everyone else; do not subtract the ante from every stack. And bounty equity: under progressive rules roughly half a collected bounty is banked and half added to the knocker's own head, so capturable equity is about half the displayed badge — inferred from a 125 → 187.5 → 281.25 progression, three data points, and worth confirming before anything depends on it.
...
**Validated in production.** The rebuild of `MPBLfM4mwfE` was the first live run of the addendum, and it holds up. The figures in this block are that run's — 63 rows, 285 seats — and are historical; the video has been re-detected twice since. See "Corpus state."
...
### Step D is given the FVA's position, not asked to find it

`build_fva_context` emits `fva_time_seconds` — video-absolute, in both `MM:SS` and raw seconds, with the frame of reference stated — and `extract_player_actions.md` tells the model to begin recording at that moment. It previously emitted only the FVA's description (seat, label, action type, amount) and asked the model to record the action "when you observe this action in the clip," which is a request to *re-locate* something Phase 4 had already located, by matching a description across a window that can run a minute or more.

Two prompts answering the same question is an invitation to disagree, and they did — see "Pre-FVA folds are recorded inconsistently." Making D trust Phase 4's answer means there is one answer, and when it is wrong the fault is localised to `identify_hand_start.md`, the prompt responsible for that job.

**The accepted risk is that a bad FVA timestamp now propagates instead of being silently corrected by D.** That is a fair trade: a silent correction is indistinguishable from a silent corruption, and the corpus has nine of the latter and no way to count the former. It is also an argument for the cross-check on the follow-up list rather than against it — if a bad timestamp propagates, that check is what surfaces it.

**This change is reasoned, not measured.** Its reproduction was a null result — the baseline did not fail in four reps — so the rate it was meant to move is unverified. See "Pre-FVA folds are recorded inconsistently" for the twelve calls and why four reps could not have settled an 8% effect.

The timestamp was confirmed video-absolute before it was emitted, since a window-relative value would anchor past the end of a short hand and a wrong anchor is worse than none. `_street_timestamp_guard` bounds D's sibling calls by the same absolute numbers passed to `types.VideoMetadata(start_offset=…)`, returned timestamps seek into the full local mp4 unadjusted, `schemas/hand_starts.json` says "not from start of clip" outright, and no rebasing arithmetic exists anywhere outside the SDK construction.
...
- **Cost is roughly $0.053 per hand** — 2.49M tokens over 204 calls, about $3.20 for the video. Input dominates. Video scans run ~20K tokens, frame reads ~2.4K, and scan cost falls with each street as the window narrows.
  - **Stale.** These are Pro-only figures from the original run, at Pro's rates. Both the rates and the per-call token counts are lower now, and there are more scan calls per hand than this assumed, so the figure is wrong in both directions rather than simply high. See "Measured call costs."
...
**Scope is correctness, not progress.** A video part-way through the pipeline is incomplete, not inconsistent, and reports clean. Conflating unprocessed with wrong would make the tool noisy from its first run, which is how an audit tool gets ignored.

**Amended: the example this paragraph used is no longer one.** It read "`YzKyFMQ1avU`'s 68 stage rows with no Phase 4 or 5 output report clean." That video has since been processed end to end and has downstream rows in both Phase 4 and Phase 5 — see "Corpus state." The principle is unchanged and the tool's behaviour is unchanged; only the illustration expired. No video in the current corpus is part-way through, so the case is now hypothetical rather than observed.
...
The rebuild appended that row, and the audit reported clean across both videos: 0 orphaned rows, 0 duplicate natural ids, 0 status/row mismatches over 260 stage rows. The finding resolved itself exactly as predicted, without a tolerance. Note what clean does *not* mean here — the same corpus was missing an estimated 5% of the video's hands and carried three phantom seats, neither of which is in scope for any of the three checks. See "Detection is not deterministic" and "Phantom seats and the checks that find them."

**The 260-row figure is historical to that run.** It predates a third `MPBLfM4mwfE` re-detection and the whole of `YzKyFMQ1avU`'s downstream processing; the corpus is larger now and the audit has not been re-run against it. See "Corpus state" for current counts. The three findings being zero is a property of the run, not a standing guarantee.
...
It tests presence, not plausibility: whether the phase can do its work with the given input, not whether the input is correct. `MPBLfM4mwfE_011_002` recorded `pot_size_bb = 0.38` where 1.88 is expected — a misread — but its stacks, seat count and hole cards were fine and Phase 4 processed it correctly. Rejecting it would have discarded a usable hand to protect a field nothing reads.

That id is historical to the 63-row rebuild and does not resolve against the current corpus, which records 0 pot mismatches; see "Corpus state." The argument does not depend on the instance surviving.
...
### The truncation rate, and why hand shape decides what it costs

A hand is **truncated** when its latest attempt carries `D reported <street> but 2 scans found none`. Measured across both videos on Flash, split by hand shape:

| video | shape | hands | truncated | rate |
|---|---|---:|---:|---:|
| `MPBLfM4mwfE` | contested | 40 | 0 | 0% |
| `MPBLfM4mwfE` | runout | 12 | 6 | 50% |
| `YzKyFMQ1avU` | contested | 71 | 7 | 10% |
| `YzKyFMQ1avU` | runout | 9 | 7 | 78% |

**15% overall, on Flash.** Shape is derived, not stored: a runout street is one recorded with an empty `actions` array, which the prompt permits only when an all-in was called.

**The shape distinction is the whole finding.** A runout street follows a called all-in and carries no decisions. The cards still matter — for showdown evaluation and for duplicate-card detection — but nothing is lost from the action record, so a runout truncation is a gap in one field of one hand. A contested truncation is different in kind: it produces a hand that **looks complete and ends one street early**, with no way to know how much was missing. That is a wrong row inside an aggregate, not a hole in one. Pooling the two rates hides exactly the distinction that decides whether the loss is tolerable.
...
Measured on the windows the pipeline actually sends, which is the only measurement that transfers — a token count from a synthetic window predicts nothing.

| call | model | prompt tokens |
|---|---|---:|
| clip detection (240s window) | `gemini-2.5-pro` | 68,874 |
| clip detection (240s window) | `gemini-3.8-flash` | 22,794 |
| frame read | `gemini-3.8-flash` | 2,303 |
| step D, whole hand window | `gemini-3.8-flash` | 9,593–17,968 |
| step E street scan | `gemini-3.8-flash` | 12,803–15,351 |
| step E street read | `gemini-3.8-flash` | 1,712 |

Two things worth stating alongside the table.

**Input is driven by window length, output by task complexity.** Step D returns roughly 30× the output of a scan while costing comparable input, because both send video over overlapping windows and only the answer differs in size. So the two levers are independent: shortening a window cuts input and leaves output alone; simplifying what a call must return does the reverse.
...
- **Clip-mode video scans — Pro.** Flash truncated a street on six hands that both Pro runs found — five rivers and one turn. Four were runouts where the cards are analytically inert but still needed for showdown evaluation and duplicate-card detection. **One, t=1320, had betting on the river that Flash therefore dropped entirely** — a lost decision, not a lost card. On the single-video evidence that was the entire case for Pro on clips, and it was weaker than it read: only t=1320 was verified against the broadcast, and two Pro runs agreeing is thin corroboration from a pair that also agreed with each other while both were wrong on seat labels at t=984 and t=1184. The reproduction that would separate "Flash cannot find these windows" from "Flash was unlucky six times" was never run at the time. On the same call mode the evidence pointed the other way too: Pro produced 9 FVA boundary violations to Flash's 0, fabricated a four-street runout at t=1779, and parked three hands on card reads against Flash's one.

  **This label has moved twice, and the sequence matters more than either move.** It read `Pro` on the single-video comparison above; it was downgraded to `inconclusive` when the clip default was reconsidered, because that was a stronger claim than the evidence carried; it now reads `Pro` again on evidence of a different kind. What arrived was not more of the same comparison but a measured truncation rate across two broadcasts and one case with every alternative eliminated — see "The truncation rate, and why hand shape decides what it costs" and "Pro recovers the contested truncations" below. The caveats in the bullet above are not retracted; they correctly describe what a two-run disagreement set on one video can support, which is why the question needed a different measurement rather than a rerun.

**The clip default is Flash, and it was a cost and throughput decision.** Pro's clip-mode calls measured **68,874 prompt tokens** each against roughly 2,300 for a frame read — about 30x, on the pipeline's highest-volume call type. Clip 009 hit a Vertex 429 twice at `--max-concurrent 2`, exhausted the internal backoff both times and parked, needing a manual un-park and a third run at `--max-concurrent 1`. That happened on a single video, and the quota pressure recurs on every one. Against it, the projected quality cost at a 20K-hand corpus was put at roughly 1.5% of hands with an incomplete board and a few hundred with a genuinely lost betting street: tolerable for population-tendency analysis, and bounded, where the quota pressure is not.

**Amended: that trade was priced with the wrong quality number, and the quality cost is not merely bounded.** The measured truncation rate across two videos is **15%, not 1.5%** — an order of magnitude — and it is not uniform: contested truncations run at 10% on `YzKyFMQ1avU` and produce hands that look complete and end a street early, which corrupts an aggregate rather than thinning it. "Bounded and detectable" described the runout half of the population and was applied to all of it. The cost and throughput half of this paragraph stands unchanged; what fails is the conclusion drawn from weighing it. See "The truncation rate, and why hand shape decides what it costs."

**The quality cost is detectable, which was most of why it was thought acceptable.** Step E records a D/E street disagreement in `status_message` as `D reported <street> but 2 scans found none — truncated there`, so truncated hands are a queryable population rather than silent loss. See "Scan retries fire only on disagreement with D."
...
- **the prompt and the reference images** — identical for both videos, and 111 contested streets succeeded against them;
- **frame extraction** — the scan never reached it, so ffmpeg is not implicated;
- **a sequencing error** — the turn frame shows four cards at the correct timestamp, so the street list was not shifted.
...
The two-video truncation rate and the `_003_001_001` investigation above are a second, independent line of evidence, and they are stronger on the clip-mode question specifically: a measured rate across two broadcasts, and one case with every alternative eliminated. They do not rescue this comparison — it remains one video and one adjudicator — but the clip-mode conclusion no longer rests on it. Frame reads still do.
...
Per-seat bounty is additive, and a rebuild was still chosen — on economics, not necessity. The corpus was one video at ~$3.20 end to end (a stale Pro-era figure; see "Measured call costs"), and the argument only needs the order of magnitude. Building migration machinery to avoid that optimises against a constraint that does not exist, and the machinery would have to exist and be correct before it saved anything. The rebuild also sidesteps the cascade gap above: Phase 3's clip-mode detection call is stochastic, so a full reprocess can change the number or timing of detected setups, and truncating `hand_setups`, `hand_starts` and `hand_actions` first avoids both the orphan case and the silently-stale case entirely.
...
**A third run has since happened, and it moved further than the first two.** `MPBLfM4mwfE` now holds 57 `hand_setups` against the 65 and 63 above — a drop of about 10% from the second run, where the first-to-second move was about 3%. The per-timestamp comparison that produced the bullets above has **not** been redone for it, so nothing here says which hands went or whether the losses are the same ones; the count is recorded and the analysis is not extended. Two things follow. The "roughly 5%" figure is the two-run measurement and should be read as the low end of a range rather than a settled rate. And the direction is consistent across all three runs — each re-detection of this video has found fewer setups than the last, which a stochastic-in-both-directions account does not predict and which the open question below should be framed to test. See "Corpus state."
...
**Amended: it has now been exercised, and one case exhausted it.** Three 429 incidents arrived in a single session — twice on Pro clip calls at `--max-concurrent 2`, once on Flash during a 96-hand Phase 4 where roughly 85% of calls failed. So the backoff does catch real 429s and sleep, which is what this paragraph said was unproven. What the incidents also show is that it is not sufficient: one clip exhausted all five attempts and parked. The open question is no longer whether the fix works but whether its patience is enough, and that is a different question with a different answer. See "Vertex uses dynamic shared quota, and the console does not show it."
...
### Vertex uses dynamic shared quota, and the console does not show it

Three 429 incidents in one session, and the console is actively misleading about why. The per-model request quotas showed **~0.01% usage while calls were being refused**, because Vertex allocates capacity from a **dynamic shared quota** — a pool distributed across users, not a fixed per-project limit. There is no number on the quota page that corresponds to the limit being hit, because there is no per-project limit to read.

This is recorded because the natural first move on a 429 is to look for a saturated quota, find 0.01%, and conclude the error must be something else. It is not something else.

The three incidents:

- **Twice on Pro clip calls at `--max-concurrent 2`.** One parked a clip, which needed manual un-parking and a third run at `--max-concurrent 1`.
- **Once on Flash during a 96-hand Phase 4**, where roughly **85% of calls failed**. The run was killed deliberately rather than letting entities burn attempts toward the retry cap — at that failure rate the cap would have parked a large fraction of a healthy video for a condition none of its entities caused.

**The backoff is correct and its patience is thin.** Truncated exponential with full jitter, capped at 60s, which matches Google's documented recommendation. But it stops after `_RETRY_MAX_ATTEMPTS` (5), giving four sleeps drawn from `U(0, min(5·2ⁿ, 60))` — an **expected total wait of about 65 seconds**, and about 130s worst case. Google's own example carries no stop condition at all. Whether to raise the attempt count is a judgment call and is **not made here**; the figure is recorded so the trade is visible rather than buried in a constant. Note the document states the attempt count only in a Phase 3 file-list bullet, which is not where a reader looks for it.

**And there is no delay between runs.** The entity-level retry cap counts consecutive failures with no cooldown, so an immediate re-run against still-saturated capacity converts pending entities into parked ones — the failure is attributed to the entity, which did nothing wrong, exactly as `blocked_upstream` exists to avoid in Phase 2. That is manageable for a CLI invoked by hand, where the operator sees the 429s and waits. It is a real gap for unattended daily operation, and the orchestration work must cover it: either a cooldown between runs, or a 429-aware status outside the `failed%` family so the cap does not advance. See CLAUDE.md's "Attempt status semantics" on why the naming is what excludes a status from the cap.
...
The change was framed as writing down a convention raises already followed, extended to the one action type applying it inconsistently — on the evidence that both stored Pro runs recorded the inclusive 7 at t=584. **Reproduction contradicted that framing.** Pro on the old prompt produced the exclusive 4.55 on the same raise, so the corpus's raise values were not following a settled convention; they were landing on one of two readings, run by run, and the all-ins were only the place it was visible. This cuts both ways. The ambiguity was wider than believed, which strengthens the case for closing it. But it also means the stored corpus cannot be read as a stable baseline for raises, and the t=584 "must not change" control was measuring a stability the old prompt did not have. `identify_hand_start.md` carries the same field in its `fva` block and had the same gap in its all-in rule, so both prompts moved together — step D is handed that block as prompt context and told the FVA is preflop `action_order 1`, so a split convention would have the two halves of one record disagreeing about the same chips.

**Amended: the "FVA cross-check" this paragraph originally invoked does not exist.** The sentence above read "Phase 5's FVA cross-check compares preflop `action_order 1` against that block directly." Nothing does. The `fva` block's only programmatic use in `hand_action_processing.py` is a presence-and-non-null-label check in `check_preconditions`; after step D returns, the only validation is the `winning_positions` emptiness check and `_street_timestamp_guard`, neither of which looks at the action sequence. The block is a prompt *anchor*, not a verified constraint — which is exactly why step D could disagree with it undetected, as "Pre-FVA folds are recorded inconsistently" below records. The check is worth building; it is now on the follow-up list.

**The check has since been run as an ad-hoc query, and it returns 0 disagreements across all ~132 hands of both videos.** Three facts, deliberately kept distinct rather than collapsed into "the check passes":

- **The check is unimplemented.** Nothing in `hand_action_processing.py` compares step D's output against the `fva` block. The paragraph above and the follow-up item are both still accurate about the code.
- **The query has been run**, by hand, outside the pipeline. It is a prototype of the check the follow-up describes, so the eventual implementation has a known-good reference to validate against.
- **The result is 0 across both videos.** A zero is meaningful evidence about the current corpus, and evidence is what the follow-up list is usually short of.

Recording only the first of those three would leave the document and the operational reality reading as contradictory to anyone comparing notes; recording only the third would claim a check exists that does not.

**What the zero does and does not mean.** It means step D's first preflop action agrees with Phase 4's `fva` block on every hand. It does **not** mean either is correct: both readings derive from the same window, so a wrong FVA that step D then reads from the same wrong moment agrees with itself. This is a consistency test between two readings, not a correctness test against the broadcast — which is the same limitation, from the other direction, as "Step D is given the FVA's position, not asked to find it" records when it accepts that a bad FVA now propagates.
...
It can be inferred, though. Summing `stack_size` across players and adding `pot_size_bb` gives a total identical between consecutive `hand_setups` rows to within ±0.2%, including across seat-count changes from bust-outs. `stack_size` is recorded *after* forced contributions have left the seats, which is why adding the pot back recovers the table total. At the setup moment `pot_size_bb` holds exactly the forced contributions and nothing else: across all 63 rebuilt rows it equalled `0.125 × total_seat_count + 1.5` — the ante per seat plus the two blinds — at every seat count from 9 down to 2. **The 0.125 is `MPBLfM4mwfE`'s ante coefficient, not the pipeline's**; see "The ante coefficient is per video." An earlier version of this section had this backwards; the identity only works because the chips have already moved. See "`bet_amount` includes a posted blind," which rests on the same fact. Four pairs depart, roughly every ten minutes, which is a level clock. The distribution is sharply bimodal: same-level pairs at 1.000 ± 0.002, level changes at ≥ 1.14, a gap of about 70× the noise floor.
...
The two remaining pairs are new and are not a level change. Chips went 71.84 → 72.27 → 71.84 entering and leaving `_012_003` at t=2751, an excursion of ±0.6% that goes up and comes straight back where a level change is a step. That is the phantom seat's 0.38 bb stack appearing and disappearing. Historical to the rebuild — that row no longer exists — but it is the worked example of conservation catching a non-null phantom, which is the one kind the null-stack precondition cannot.

### The ante coefficient is per video

`MPBLfM4mwfE` fits **0.125 exactly** at every seat count from 9 down to 2. `YzKyFMQ1avU` does not: it clusters **0.120–0.130**, computed from the modal pot at each seat count.

So a pot-versus-seat-count check must use **a per-video median fit and a tolerance**, never equality against a constant. Written as `pot_size_bb = 0.125 × seats + 1.5` it passes on one video and fails on every hand of the other, which would read as a corpus-wide extraction defect rather than as a different ante structure — the worst kind of false positive, because it is loud, uniform and wrong.

Two reasons the constant was believable. It held exactly across nine seat counts on the first video, which looks like a structural constant rather than a fitted one; and the blind level is not observable anywhere on screen (see "BB denomination and chip conservation"), so there was nothing to contradict it. The second video is the first evidence that the coefficient is a property of the tournament's structure, not of the pipeline.

`MPBLfM4mwfE`'s exactness is itself now the thing wanting explanation. A fitted value landing on 0.125 at every seat count is either a genuinely fixed big-blind ante or an artifact of the pot being rendered to two decimals; `YzKyFMQ1avU`'s spread is what a real fit looks like. Nothing here settles which, and the derived `committed[seat]` arithmetic under "Per-seat bounty capture" inherits the question.

### Phantom seats and the checks that find them

A phantom seat is a `hand_setups` row declaring one more player than the table holds. Three of the rebuilt 63 rows carried one: `_005_005` (SB), `_009_005` (CO) and `_012_003` (CO).

**Historical to the 63-row rebuild.** Those three ids are not re-derivable: `MPBLfM4mwfE` has been re-detected since, the ids renumbered, and the latest run carries **0 phantom seats** (see "Corpus state"). The defect class is real and the two checks below are still the way to find it — what has expired is the ability to look these three up. Do not read the zero as the class being fixed; detection is stochastic and a clean run is one sample. Two read null for `stack_size`, `bounty` and hole cards alike, and Phase 4's null-stack precondition filters them to `complete_skipped` — which is why the class was originally recorded as two separate defects, an "all-null stack read" and a "phantom seat." They are the same thing. `_012_003`'s extra seat reports `stack_size = 0.38` with a null bounty, passes the precondition, and Phase 4 processed the hand `complete`.

Two things found it, neither of which was designed for it and neither of which needs a schema change:

- **Seat count is non-increasing within a video.** Players bust; they do not rejoin. Comparing the player count against `LAG` over `hand_setup_time_seconds` returned five violations across the corpus as it stood — the three above, plus `YzKyFMQ1avU_009_001` and `YzKyFMQ1avU_012_006`, which at the time sat in a video Phase 4 had not yet touched. It has since been processed end to end (see "Corpus state"), so that qualification no longer holds and the two ids are subject to the same renumbering caveat as the rest. One window function on one stage table, no join, no LLM. It catches all three kinds.
...
**An observation, not a claim: 0.38 appeared twice as a spurious value.** `_011_002` recorded `pot_size_bb = 0.38` where 1.88 is expected (see "What `check_preconditions` is for"), and `_012_003`'s phantom seat reported `stack_size = 0.38`. Different fields, different rows, different runs. Almost certainly coincidence, and two instances is nothing. But if 0.38 turns out to be something rendered on screen that the model occasionally latches onto, that is a lead worth having already written down — and noting it costs nothing. A third sighting in any field is the point at which it stops being a coincidence.

Both sightings are historical to the 63-row rebuild and neither row exists today. The count stands at two: a later run not reproducing them is not a third sighting, and is not evidence against the lead either.
...
The denominator is the finding. Note the three figures are not on one scale — the first two count hands and nulls, the third counts seats — so they do not divide into a ratio. The point is only that the great majority of nulls are correct, and that the seat-level rate a naive query returns is therefore not a quality figure at all.

Three observed causes:

- **frame-limited illegibility** — already documented; six reproductions against the stored frame returned null every time, so the card is genuinely not legible at the FVA moment. See "What `hand_starts` guarantees, and what it does not."
...
**Measured since, across the stored corpus: 9 of 117 Pro hands over the two runs open preflop with a fold at `action_order 1`.** Seven of the nine are BTN, two CO, one UTG+1 — not evenly spread across positions. Only one of the nine appears in *both* Pro runs, so the same window produces the violation on some calls and not others; it is stochastic, not a property of particular hands. Flash returned 0 of 58. **That zero is not a demonstrated difference** — the denominator is half Pro's, the expected count at Pro's rate is about four, and no hand was adjudicated to check whether Flash was right for the right reason. It is a lead, not a finding.

`MPBLfM4mwfE_010_004_001` (hand setup t=2359) is the case adjudicated against the broadcast: BTN folded, SB completed, BB checked. Phase 4 got it right — `hand_start_state.fva` reads `{action_type: call, bet_amount: 1, seat_position_label: SB}` — and step D, handed exactly that context, recorded `1. BTN fold` anyway. So the instruction was correct, the data handed to the model was correct, and the model started one action early regardless.

**Changed, but not demonstrated fixed.** `build_fva_context` now emits `fva_time_seconds` alongside the seat and action, and `extract_player_actions.md` directs the model to begin recording *there* rather than "when you observe this action in the clip," with pre-FVA actions excluded in so many words — see "Step D is given the FVA's position, not asked to find it" under Phase 5. The reasoning is sound on its own terms (one prompt owns locating the FVA instead of two), and the change is cheap and safe. **What it is not is evidence.**

**The regression guard came back empty, and the honest reading is that nothing was shown.** Twelve Pro calls against `_010_004_001`'s real window, three arms of four: A old prompt + old context, B old prompt + new context (the timestamp anchor alone), C new prompt + new context.

- **A returned the correct `SB call 1` on all four reps.** The baseline failure did not reproduce, so there was no defect for B or C to repair, and C's clean 4/4 says nothing the null result does not already explain.
- **The only violation in all twelve calls was B2** — `BTN fold 0.0` at `action_order 1`, from the arm carrying the new timestamp. At four reps 1/4 against 0/4 is indistinguishable from noise and should not be read as the anchor causing harm. But it is squarely inconsistent with the anchor being sufficient on its own, which was the hypothesis B existed to test.

So the corpus rate is roughly 8% and a 4-rep arm has about a 70% chance of showing nothing even when the defect is live — the design was underpowered for the effect it was measuring, and that was foreseeable from the 9-in-117 figure before the calls were made. **Do not record this change as validated.** The load-bearing guard against pre-FVA folds remains the unbuilt cross-check on the follow-up list, not this reproduction.

The two instructions this section names as jointly responsible — `# ACTION ORDER` and the fold-is-a-recordable-action list — were left alone. Example 9 already shows the FVA at `action_order 1` with no pre-FVA folds, so it supports the target reading rather than opposing it, and the explicit exclusion sentence addresses the contradiction without a wider rewrite. That remains the next lever if the rate does not move; B2 is a weak argument for pulling it.
