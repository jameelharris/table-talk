# Schema source of truth: schemas/hand_start_processing_attempts.json
# HandStartProcessingAttemptsRow is generated from that schema by
...
class HandStartProcessingAttemptsWriteError(Exception):
    pass


def write_hand_start_processing_attempt_row(
    ...
    *,
    ...
    if row.status not in VALID_STATUSES:
        raise HandStartProcessingAttemptsWriteError(
    ...
    table = f"{project}.{dataset}.hand_start_processing_attempts"
    ...
    try:
        ...
        raise HandStartProcessingAttemptsWriteError(str(exc)) from exc
    if job.errors:
        raise HandStartProcessingAttemptsWriteError(f"BQ DML errors: {job.errors}")


def write_hand_start_processing_attempt_rows(
    rows: list[HandStartProcessingAttemptsRow],
    *,
    project: str,
    dataset: str,
    client: bigquery.Client | None = None,
    ...
    The batched sibling of write_hand_start_processing_attempt_row, for `tt mark-pending`,
